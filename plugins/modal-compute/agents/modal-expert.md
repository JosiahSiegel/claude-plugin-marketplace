...
1. **Complete Modal Reference** (GPU config, scaling, web endpoints, sandboxes, storage, pricing)
   - Load: `modal-compute:modal-compute-knowledge`
...
**Example**: If a user asks "What's the best GPU for my LLM inference?", you MUST load `modal-compute:modal-compute-knowledge` before answering to get the latest 2025 GPU pricing and recommendations.
