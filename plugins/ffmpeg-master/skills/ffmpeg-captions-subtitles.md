...
description: Complete subtitle and caption system. PROACTIVELY activate for: (1) Burning subtitles (hardcoding SRT/ASS/VTT), (2) Adding soft subtitle tracks, (3) Extracting subtitles from video, (4) Subtitle format conversion, (5) Styled captions (font, color, outline, shadow), (6) Subtitle positioning and alignment, (7) CEA-608/708 closed captions, (8) Text overlays with drawtext, (9) Whisper AI transcription (FFmpeg 8.0+), (10) Batch subtitle processing. Provides: Format reference tables, styling parameter guide, position alignment charts, dynamic text examples, accessibility best practices. Ensures: Professional captions with proper styling and accessibility compliance.
---
...
```bash
# Generate subtitles using Whisper (FFmpeg 8.0+)
ffmpeg -i video.mp4 \
  -vf "whisper=model=base" \
  -c:a copy \
  output_with_subs.mp4

# Whisper with language detection
ffmpeg -i video.mp4 \
  -vf "whisper=model=small:language=auto" \
  -c:a copy \
  output.mp4

# Export Whisper transcription to SRT
ffmpeg -i video.mp4 \
  -vf "whisper=model=base:output_format=srt" \
  -f null - 2> transcript.srt
```
