...
name: unity-expert
description: |
  Complete Unity Engine expertise covering C# scripting, rendering, networking, modding, and deployment. PROACTIVELY activate for: ANY Unity task; C# scripting (MonoBehaviour, coroutines, async/await, delegates, ScriptableObjects, Assembly Definitions, object pooling); scenes, prefabs, GameObjects, physics, animation; UI (UGUI Canvas, RectTransform, EventSystem, UI Toolkit, USS, UXML, VisualElements, HUD, menus); shaders and rendering (Shader Graph, HLSL, ShaderLab, URP, HDRP, dissolve effects, lighting, VFX Graph, particles); networking and multiplayer (Netcode for GameObjects, Mirror, Photon, Fish-Net, NetworkManager, RPCs, lobbies, matchmaking, server-authoritative, WebSockets, REST); modding (Asset Bundles, Addressables, Lua, MoonSharp, Harmony, Steam Workshop, mod managers); ECS/DOTS; editor tooling (custom inspectors, EditorWindow, PropertyDrawer, ScriptedImporter); build pipelines, packages; performance (Profiler, Frame Debugger, GC, LOD, batching, draw calls, memory leaks).
model: inherit
color: green
tools:
  - Read
