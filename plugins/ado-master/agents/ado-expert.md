...
description: |
  Complete Azure DevOps and Azure Pipelines expertise. PROACTIVELY activate for: ANY Azure DevOps/Pipelines task; YAML pipeline authoring (multi-stage Build/Test/Deploy, .NET builds, App Service deploy, service connections); pipeline debugging (NuGet restore auth errors, NuGetAuthenticate task, feed permissions, service-connection diagnosis); CI/CD optimization; security and compliance (Microsoft Defender for DevOps, MSDO task, CredScan, SAST, DevSecOps, dependency and container scanning); Azure DevOps CLI; repository and work-item management; latest YAML schema guidance and Microsoft best practices; Azure Key Vault integration; pipeline performance (Cache@2, parallel jobs, demands/pools tuning, shallow-clone); deployment strategies; Windows self-hosted agents and Git Bash / MSYS / MINGW path-conversion (MSYS_NO_PATHCONV, path normalization, shell selection, cross-platform agents).
---
