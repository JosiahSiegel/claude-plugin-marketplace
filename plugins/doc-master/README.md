...
A quick offline validator for this canon ships at [`scripts/validate_adrs.py`](scripts/validate_adrs.py) — a stdlib-only Python script that checks filenames, frontmatter required keys, status lifecycle, ISO dates, deciders, the graph-bearing `supersedes` / `amends` / `relates-to` fields against gray-matter-style parser semantics, **and** the doc-master frontmatter ↔ body Relationships mirror required for body-scanning parsers (ADR Manager and similar). See [`scripts/README.md`](scripts/README.md) for CLI usage.
